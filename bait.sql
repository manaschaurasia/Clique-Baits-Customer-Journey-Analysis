...
/* 3 */
Select month(e.event_time) as Month_Num ,monthname(e.event_time) as Month_Name,count(DISTINCT visit_id) as tot_number_of_visits
FROM users u
LEFT JOIN events e
...
/* 4 */
SELECT e.event_type, ei.event_name , COUNT(e.visit_id)
FROM events e 
...
SELECT 
    ROUND(((SELECT 
        ...
        FROM
            events
        WHERE
            event_type = 1)) * 100, 2) AS Purchase_percentage;
...
SELECT  p.page_name, COUNT(e.page_id) AS Num_of_visits
FROM events e
